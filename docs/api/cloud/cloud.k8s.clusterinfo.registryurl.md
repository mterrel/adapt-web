...
If `registry` is a string, it is assumed that the cluster can pull from the same string that outsiders can push to.

If `registry` is of the form `{ external: string, internal: string }` then the `external` string will be used to push images, and the `internal` string will be used to pull images.
